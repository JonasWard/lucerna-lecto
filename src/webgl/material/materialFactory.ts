...
import { Color, Material, Scene } from 'three';
...
export class MaterialFactory {
  private static create = (scene: Scene, materialOptions: MaterialOptions): Material => {
    const material = scene.getObjectByName(materialOptions.name);
  ...
  static colorFromHexString = (s: string): Color => new Color(Number.parseInt(s.slice(1), 16));

  // red hui
  private static getColorFromUUIDRed = (s: string) => MaterialFactory.colorFromHexString(`#ff${s.slice(0, 2)}${s.slice(0, 2)}`);
  // green hui
  private static getColorFromUUIDGreen = (s: string) => MaterialFactory.colorFromHexString(`#${s.slice(0, 2)}ff${s.slice(0, 2)}`);
  // blue hui
  private static getColorFromUUIDBlue = (s: string) => MaterialFactory.colorFromHexString(`#${s.slice(0, 2)}${s.slice(0, 2)}ff`);
  // normal colors
  private static getColorFromUUIDRGB = (s: string) => MaterialFactory.colorFromHexString(`#${s.slice(0, 6)}`);
  // black and white
  private static getColorFromUUIDBlackAndWhite = (s: string) => MaterialFactory.colorFromHexString(`#${s.slice(0, 2)}${s.slice(0, 2)}${s.slice(0, 2)}`);

  private static maxValue = 2 ** 24 - 1;
  private static inverseMaxValue = 1 / MaterialFactory.maxValue;

  //
  private static getColorInArray = (s: string, a: [number, number, number][]): Color => {
    const v = parseInt(s, 16) * MaterialFactory.inverseMaxValue;
    const index = v * (a.length - 1);
    const i0 = Math.floor(index);
    const i1 = Math.ceil(index);

    if (i0 === i1) return new Color(...a[i0]);
    const f = index - i0;
    return new Color(a[i0][0] * (1 - f) + a[i1][0] * f, a[i0][1] * (1 - f) + a[i1][1] * f, a[i0][2] * (1 - f) + a[i1][2] * f);
  };
  ...
  public static getViridisColorFromUnitValue = (v: number) => MaterialFactory.getColorFromUnitValue(v, VIRIDIS).getHexString();
  public static getTwilightColorFromUnitValue = (v: number) => MaterialFactory.getColorFromUnitValue(v, TWILIGHT).getHexString();
  public static getPlasmaColorFromUnitValue = (v: number) => MaterialFactory.getColorFromUnitValue(v, PLASMA).getHexString();
  ...
  private static lampMaterialOptions: MaterialOptions = {
    name: 'lampMaterial',
    emissiveColor: new Color(0.67, 0.64, 0.49),
    wireframe: false,
  };
