...
export const exportSTL = (mesh: Mesh, fileName = 'lucerna-lecto') => {
  ...
  for (let i = 0; i < vertexData.indices.length; i += 3) {
    ...
    if (f.length === 3) {
      const v0 = V3.fromNumbers(
        vertexData.positions[f[0] * 3],
        vertexData.positions[f[0] * 3 + 1],
        vertexData.positions[f[0] * 3 + 2]
      )
      const v1 = V3.fromNumbers(
        vertexData.positions[f[1] * 3],
        vertexData.positions[f[1] * 3 + 1],
        vertexData.positions[f[1] * 3 + 2]
      )
      const v2 = V3.fromNumbers(
        vertexData.positions[f[2] * 3],
        vertexData.positions[f[2] * 3 + 1],
        vertexData.positions[f[2] * 3 + 2]
      )
...
outer loop
vertex ${v0.x.toPrecision(STL_NUMBER_PRECISION)} ${v0.y.toPrecision(STL_NUMBER_PRECISION)} ${v0.z.toPrecision(
          STL_NUMBER_PRECISION
        )}
vertex ${v1.x.toPrecision(STL_NUMBER_PRECISION)} ${v1.y.toPrecision(STL_NUMBER_PRECISION)} ${v1.z.toPrecision(
          STL_NUMBER_PRECISION
        )}
vertex ${v2.x.toPrecision(STL_NUMBER_PRECISION)} ${v2.y.toPrecision(STL_NUMBER_PRECISION)} ${v2.z.toPrecision(
          STL_NUMBER_PRECISION
        )}
endloop
endfacet`
