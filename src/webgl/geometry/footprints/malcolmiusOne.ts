...
import { FootprintMalculmiusOne } from '../../../modelDefinition/types/version0.generatedType';
...
export const createFootprintMesh = (malcolmiusOne: FootprintMalculmiusOne): SimpleMesh => {
  // creating the base profiles
  const circleRadius = malcolmiusOne[AttributeNames.CircleRadius].value;
  const circleDivisions = malcolmiusOne[AttributeNames.CircleDivisions].value;
  const angleSplit = malcolmiusOne[AttributeNames.AngleSplit].value;
  const offsetA = malcolmiusOne[AttributeNames.OffsetA].value;
  const offsetB = malcolmiusOne[AttributeNames.OffsetB].value;
  const innerRadius = malcolmiusOne[AttributeNames.InnerRadius].value;
  ...
  return SimpleMesh.joinMeshes(vertexSets.map((vertices) => SimpleMesh.makeFromPolygon(vertices)));
};

export const createFootprintHalfedgeMesh = (malcolmiusOne: FootprintMalculmiusOne) =>
