  ...
  const faceCenterIdxs: number[] = mesh.faces.map((f, fIdx) => {
    ...
    faceMidEdgeArray.push(
      f.map((vIdx, i, arr) => {
        ...
        const localName =
          vIdx < arr[(i + 1) % arr.length]
            ? `${vIdx}-${arr[(i + 1) % arr.length]}`
            : `${arr[(i + 1) % arr.length]}-${vIdx}`
        if (edgeIdxVCollectiondMap[localName] === undefined)
          edgeIdxVCollectiondMap[localName] = [vIdx, arr[(i + 1) % arr.length], fCenterIdx]
        else edgeIdxVCollectiondMap[localName].push(fCenterIdx)

        // handling of the individual vertices
        if (vertexNeighbourMap[vIdx] === undefined)
          vertexNeighbourMap[vIdx] = new Set([arr[(i + 1) % arr.length], arr[(i + arr.length - 1) % arr.length]])
        else {
  ...
  faceMidEdgeArray.forEach((heIdxs, i) => {
    ...
    f.forEach((vIdx, j) =>
      faces.push([
        ...
        edgeVId[heIdxs[j]],
        faceCenterIdxs[i],
        edgeVId[heIdxs[(j + heIdxs.length - 1) % heIdxs.length]],
      ])
    )
  })
  ...
  mesh.faces.forEach((f) => {
    const midEdges = f.map((vIdx, i, arr) => {
      ...
      if (edgeVertices[localName] === undefined) {
        edgeVertices[localName] =
          vertices.push(V3.mul(V3.add(vertices[vIdx], vertices[arr[(i + 1) % arr.length]]), 0.5)) - 1
      ...
      if (allMidsPerVertex[arr[(i + 1) % arr.length]])
        allMidsPerVertex[arr[(i + 1) % arr.length]].push(edgeVertices[localName])
