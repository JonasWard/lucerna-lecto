...
export const sdGyroid = (v: V3, scale = 1) =>
  ...
  0.666666
export const sdSchwarzP = (v: V3, scale = 1) =>
  (Math.cos(v.x * scale) + Math.cos(v.y * scale) + Math.cos(v.z * scale)) * 0.333333
export const sdSchwarzD = (v: V3, scale = 1) =>
  Math.cos(v.x * scale) * Math.cos(v.y * scale) * Math.cos(v.z * scale) -
  Math.sin(v.x * scale) * Math.sin(v.y * scale) * Math.sin(v.z * scale)
export const sdNeovius = (v: V3, scale = 1) =>
  (3 * (Math.cos(v.x * scale) + Math.cos(v.y * scale) + Math.cos(v.z * scale)) -
    4 * Math.cos(v.x * scale) * Math.cos(v.y * scale) * Math.cos(v.z * scale)) *
  0.1428571429

export const sdSphere = (v: V3, scale = 1, c: V3 = V3.fromNumbers(0, 0, 0)) => V3.getLength(V3.sub(v, c)) * scale - 1
export const sdBox = (v: V3, scale = 1) =>
  Math.max(Math.abs(v.x * scale), Math.abs(v.y * scale), Math.abs(v.z * scale)) - 1
