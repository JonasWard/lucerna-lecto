...
export class SimpleMesh {
  public static makeLoft = (loftOption: LoftOption, ...vss: V3[][]): SimpleMesh => {
    ...
    const baseIndexes = [
      ...Array(
        loftOption === LoftOption.Open || loftOption === LoftOption.Vloop ? vss[0].length - 1 : vss[0].length
      ).keys(),
    ].map((i) => [i, (i + 1) % vss[0].length])

    const faces = [
      ...Array(loftOption === LoftOption.Open || loftOption === LoftOption.Uloop ? vss.length - 1 : vss.length).keys(),
    ]
      .map((j) => {
        const jBis = (j + 1) % vss.length
        return baseIndexes.map(([v0, v1]) => [
          v0 + j * vss[0].length,
          v1 + j * vss[0].length,
          v1 + jBis * vss[0].length,
          v0 + jBis * vss[0].length,
        ])
      })
      .flat()
