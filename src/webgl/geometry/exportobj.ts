...
export const exportOBJ = (mesh: Mesh, fileName = 'lucerna-lecto') => {
  ...
  const positionStrings = [...Array(vertexData.positions.length / 3).keys()]
    .map(
      (i) =>
        `v ${vertexData.positions[i * 3].toFixed(OBJ_PRECISION)} ${vertexData.positions[i * 3 + 1].toFixed(
          OBJ_PRECISION
        )} ${vertexData.positions[i * 3 + 2].toFixed(OBJ_PRECISION)}`
    )
    .join('\n')
  ...
  const faceStrings = [...Array(vertexData.indices.length / 3).keys()]
    .map(
      (i) =>
        `f ${vertexData.indices[i * 3] + 1}/${vertexData.indices[i * 3] + 1} ${vertexData.indices[i * 3 + 1] + 1}/${
          vertexData.indices[i * 3 + 1] + 1
        } ${vertexData.indices[i * 3 + 2] + 1}/${vertexData.indices[i * 3 + 2] + 1}`
    )
    .join('\n')
